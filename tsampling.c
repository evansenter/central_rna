...
int tsampleZBBL(int i, int j, double **Z, double **ZB, double **ZBB, double **ZM, double **ZM1, double **ZBBL, double **ZBBR, char *str, long *seed, int *seq_int){
  ...
  if(rd<cumulative)
    return tsampleZBB(i+2,j-1,Z,ZB,ZBB,ZM,ZM1,ZBBL,ZBBR,str,seed,seq_int);
  ...
  if(rd<cumulative)
    return tsampleZBBL(i+2,j-1,Z,ZB,ZBB,ZM,ZM1,ZBBL,ZBBR,str,seed,seq_int);
  ...
  for(k = i+5; (k <= j-SIGMA-3); k++){
    ...
    if(rd<cumulative)
       return tsampleZB(k,j-2,Z,ZB,ZBB,ZM,ZM1,ZBBL,ZBBR,str,seed,seq_int);
  }
  ...
  if(rd<cumulative)
    return tsampleZBBR(i+2,j-1,Z,ZB,ZBB,ZM,ZM1,ZBBL,ZBBR,str,seed,seq_int);
  ...
  for(k = j-4; (k>=i+SIGMA+4); k--){
    ...
    if(rd<cumulative)
       return tsampleZB(i+3,k,Z,ZB,ZBB,ZM,ZM1,ZBBL,ZBBR,str,seed,seq_int);
  }
  ...
  for(l = i+4; (l <= j-SIGMA-4)&&(l-i-1<=30); l++){
    for(r = j -3; (r>=l+SIGMA+1)&&(j-r+l-i-1<=30); r--){
      ...
      if(rd<cumulative)
       return tsampleZB(l,r,Z,ZB,ZBB,ZM,ZM1,ZBBL,ZBBR,str,seed,seq_int);
    }
  }
  ...
  for(k = i+SIGMA+5; k <= j-SIGMA-3; k++){
    ...
    if(rd<cumulative){
       tsampleZM(i+3,k-1,Z,ZB,ZBB,ZM,ZM1,ZBBL,ZBBR,str,seed,seq_int);
       tsampleZM1(k,j-2,Z,ZB,ZBB,ZM,ZM1,ZBBL,ZBBR,str,seed,seq_int);
       return 0;
    }
  }
  //printf("Nothing? rd = %f\n",rd);
...
int tsampleZBBR(int i, int j, double **Z, double **ZB, double **ZBB, double **ZM, double **ZM1, double **ZBBL, double **ZBBR, char *str, long *seed, int *seq_int){
  ...
  if(rd<cumulative)
    return tsampleZBB(i+1,j-2,Z,ZB,ZBB,ZM,ZM1,ZBBL,ZBBR,str,seed,seq_int);
  ...
  if(rd<cumulative)
    return tsampleZBBL(i+1,j-2,Z,ZB,ZBB,ZM,ZM1,ZBBL,ZBBR,str,seed,seq_int);
  ...
  for(k = i+4; (k <= j-SIGMA-4); k++){
    ...
    if(rd<cumulative)
       return tsampleZB(k,j-3,Z,ZB,ZBB,ZM,ZM1,ZBBL,ZBBR,str,seed,seq_int);
  }
  ...
  if(rd<cumulative)
    return tsampleZBBR(i+1,j-2,Z,ZB,ZBB,ZM,ZM1,ZBBL,ZBBR,str,seed,seq_int);
  ...
  for(k = j-5; (k>=i+SIGMA+3); k--){
    ...
    if(rd<cumulative)
       return tsampleZB(i+2,k,Z,ZB,ZBB,ZM,ZM1,ZBBL,ZBBR,str,seed,seq_int);
  }
  ...
  for(k = i+SIGMA+4; k <= j-SIGMA-4; k++){
    ...
    if(rd<cumulative){
       tsampleZM(i+2,k-1,Z,ZB,ZBB,ZM,ZM1,ZBBL,ZBBR,str,seed,seq_int);
       tsampleZM1(k,j-3,Z,ZB,ZBB,ZM,ZM1,ZBBL,ZBBR,str,seed,seq_int);
       return 0;
    }
  }
  //printf("Nothing? rd = %f\n",rd);
...
int tsampleZBB(int i, int j, double **Z, double **ZB, double **ZBB, double **ZM, double **ZM1, double **ZBBL, double **ZBBR, char *str, long *seed, int *seq_int){
  ...
  if(rd<cumulative)
    return tsampleZBB(i+1,j-1,Z,ZB,ZBB,ZM,ZM1,ZBBL,ZBBR,str,seed,seq_int);
  ...
  if(rd<cumulative)
    return tsampleZBBL(i+1,j-1,Z,ZB,ZBB,ZM,ZM1,ZBBL,ZBBR,str,seed,seq_int);
  ...
  if(rd<cumulative)
    return tsampleZBBR(i+1,j-1,Z,ZB,ZBB,ZM,ZM1,ZBBL,ZBBR,str,seed,seq_int);
...
int tsampleZB(int i, int j, double **Z, double **ZB, double **ZBB, double **ZM, double **ZM1, double **ZBBL, double **ZBBR, char *str, long *seed, int *seq_int){
  ...
  if(rd<cumulative)
    return tsampleZBB(i,j,Z,ZB,ZBB,ZM,ZM1,ZBBL,ZBBR,str,seed,seq_int);
  ...
  for(k = i+SIGMA+3; k <= j-SIGMA-2; k++){
    ...
    if(rd<cumulative){
       tsampleZM(i+1,k-1,Z,ZB,ZBB,ZM,ZM1,ZBBL,ZBBR,str,seed,seq_int);
       tsampleZM1(k,j-1,Z,ZB,ZBB,ZM,ZM1,ZBBL,ZBBR,str,seed,seq_int);
       return 0;
    }
  }
  //printf("Nothing? rd = %f\n",rd);
...
int tsampleZM1(int i, int j, double **Z, double **ZB, double **ZBB, double **ZM, double **ZM1, double **ZBBL, double **ZBBR, char *str, long *seed, int *seq_int){
  ...
  for(k = i+SIGMA+1; k <= j; k++){
    ...
    if(rd<cumulative)
      return tsampleZB(i,k,Z,ZB,ZBB,ZM,ZM1,ZBBL,ZBBR,str,seed,seq_int);
  }
  //printf("Nothing? rd = %f\n",rd);
}
...
int tsampleZM(int i, int j, double **Z, double **ZB, double **ZBB, double **ZM, double **ZM1, double **ZBBL, double **ZBBR, char *str, long *seed, int *seq_int){
  ...
  for(k = i; k <= j-SIGMA-1; k++){
    ...
    cumulative += aux;
    if(rd<cumulative)
      return tsampleZM1(k,j,Z,ZB,ZBB,ZM,ZM1,ZBBL,ZBBR,str,seed,seq_int);
  }
  ...
  for(k = i; k <= j-SIGMA-1; k++){
    ...
    cumulative += aux;
    if(rd<cumulative){
      tsampleZM(i,k,Z,ZB,ZBB,ZM,ZM1,ZBBL,ZBBR,str,seed,seq_int);
      tsampleZM1(k+1,j,Z,ZB,ZBB,ZM,ZM1,ZBBL,ZBBR,str,seed,seq_int);
      return 0;
    }
  }
  //printf("Nothing? rd = %f\n",rd);
...
int tsampleZ(int i, int j, double **Z, double **ZB, double **ZBB, double **ZM, double **ZM1, double **ZBBL, double **ZBBR, char *str, long *seed, int *seq_int){
  ...
  if(rd<cumulative)
    return tsampleZ(i,j-1,Z,ZB,ZBB,ZM,ZM1,ZBBL,ZBBR,str,seed,seq_int);
   
  cumulative+= ceil(cbp[ij_index]*ZB[i][j]/Z[i][j]*MAX_PREC);
  if(rd<cumulative)
    return tsampleZB(i,j,Z,ZB,ZBB,ZM,ZM1,ZBBL,ZBBR,str,seed,seq_int);

  for(k = i+1;k<=j-SIGMA-1;k++){ 
    kj_index = d1[GetIndex(seq_int[k],seq_int[j])];
    cumulative += ceil(cbp[kj_index]*Z[i][k-1]*ZB[k][j]/Z[i][j]*MAX_PREC);
    if(rd<cumulative){
      tsampleZ(i,k-1,Z,ZB,ZBB,ZM,ZM1,ZBBL,ZBBR,str,seed,seq_int);
      tsampleZB(k,j,Z,ZB,ZBB,ZM,ZM1,ZBBL,ZBBR,str,seed,seq_int);
      return 0;
    }
  }
  //printf("Nothing? rd = %f\n",rd);
...
double tsample(int *seq_int, int n, int samples){ 
  double **Z;
  double **ZB;
  double **ZBB;
  double **ZBBL;
  ...
  for (d = SIGMA+1; d <= n; d++){
    for(i=1; (i+d<=n); i++){
      ...
      ZBBL[j][i] = tcalculateSZBBLpfns(seq_int,i,j,ZBB,ZB,ZM,ZM1,ZBBL,ZBBR);

      ZBBR[j][i] = tcalculateSZBBRpfns(seq_int,i,j,ZBB,ZB,ZM,ZM1,ZBBL,ZBBR);
 
      ZBB[j][i] = tcalculateSZBBpfns(seq_int,i,j,ZBB,ZB,ZM,ZM1,ZBBL,ZBBR);

      ZB[j][i] = tcalculateSZBpfns(seq_int,i,j,ZBB,ZB,ZM,ZM1,ZBBL,ZBBR);

      ZM1[j][i] = tcalculateSZM1pfns(seq_int,i,j,ZB,ZM1);

      ZM[j][i] = tcalculateSZMpfns(seq_int,i,j,ZM1,ZM);
  ...
  //Sampling
  for(i = 0; i<samples;i++){
      tsampleZ(1,n,Z,ZB,ZBB,ZM,ZM1,ZBBL,ZBBR,str,&seed,seq_int);
      for(j = 1 ; j<=n; j++){
        printf("%c",str[j]);
        str[j] = '.';
      }
