...
int tbacktrackSP2ZBBL(int i, int j, double **Z, double **ZB, double **ZBB, double **ZM, double **ZM1, double **ZBBL, double **ZBBR, double **p, int samples, int *seq_int){
  ...
  tbacktrackSP2ZBB(i+2,j-1,Z,ZB,ZBB,ZM,ZM1,ZBBL,ZBBR,p,aux,seq_int);
  ...
  tbacktrackSP2ZBBL(i+2,j-1,Z,ZB,ZBB,ZM,ZM1,ZBBL,ZBBR,p,aux,seq_int);
  ...
  for(k = i+5; (k <= j-SIGMA-3); k++){
    ...
    tbacktrackSP2ZB(k,j-2,Z,ZB,ZBB,ZM,ZM1,ZBBL,ZBBR,p,aux,seq_int);
  }
  ...
  tbacktrackSP2ZBBR(i+2,j-1,Z,ZB,ZBB,ZM,ZM1,ZBBL,ZBBR,p,aux,seq_int);
  ...
  for(k = j-4; (k>=i+SIGMA+4); k--){
    ...
    tbacktrackSP2ZB(i+3,k,Z,ZB,ZBB,ZM,ZM1,ZBBL,ZBBR,p,aux,seq_int);
  }
  ...
  for(l = i+4; (l <= j-SIGMA-4)&&(l-i-1<=30); l++){
    for(r = j -3; (r>=l+SIGMA+1)&&(j-r+l-i-1<=30); r--){
      ...
      tbacktrackSP2ZB(l,r,Z,ZB,ZBB,ZM,ZM1,ZBBL,ZBBR,p,aux,seq_int);
    }
  }

  for(k = i+SIGMA+5; k <= j-SIGMA-3; k++){
    aux = floor(samples*((ZM[i+3][k-1]*ZM1[k][j-2]*exp(-1*((GetStackEnergy(i,j,i+2,j-1,seq_int)+MultiloopA+MultiloopB)/RT)))/ZBBL[i][j]));
    tbacktrackSP2ZM(i+3,k-1,Z,ZB,ZBB,ZM,ZM1,ZBBL,ZBBR,p,aux,seq_int);
    tbacktrackSP2ZM1(k,j-2,Z,ZB,ZBB,ZM,ZM1,ZBBL,ZBBR,p,aux,seq_int);   
  }
  //printf("Nothing? rd = %f\n",rd);
}                      
...
int tbacktrackSP2ZBBR(int i, int j, double **Z, double **ZB, double **ZBB, double **ZM, double **ZM1, double **ZBBL, double **ZBBR, double **p, int samples, int *seq_int){
  ...
  tbacktrackSP2ZBB(i+1,j-2,Z,ZB,ZBB,ZM,ZM1,ZBBL,ZBBR,p,aux,seq_int);
  ...
  tbacktrackSP2ZBBL(i+1,j-2,Z,ZB,ZBB,ZM,ZM1,ZBBL,ZBBR,p,aux,seq_int);
  ...
  for(k = i+4; (k <= j-SIGMA-4); k++){
    ...
    tbacktrackSP2ZB(k,j-3,Z,ZB,ZBB,ZM,ZM1,ZBBL,ZBBR,p,aux,seq_int);
  }
  ...
  tbacktrackSP2ZBBR(i+1,j-2,Z,ZB,ZBB,ZM,ZM1,ZBBL,ZBBR,p,aux,seq_int);
  ...
  for(k = j-5; (k>=i+SIGMA+3); k--){
    ...
    tbacktrackSP2ZB(i+2,k,Z,ZB,ZBB,ZM,ZM1,ZBBL,ZBBR,p,aux,seq_int);
  }
  ...
  for(k = i+SIGMA+4; k <= j-SIGMA-4; k++){
    ...
    tbacktrackSP2ZM(i+2,k-1,Z,ZB,ZBB,ZM,ZM1,ZBBL,ZBBR,p,aux,seq_int);
    tbacktrackSP2ZM1(k,j-3,Z,ZB,ZBB,ZM,ZM1,ZBBL,ZBBR,p,aux,seq_int);
  }
  //printf("Nothing? rd = %f\n",rd);
}                      
...
int tbacktrackSP2ZBB(int i, int j, double **Z, double **ZB, double **ZBB, double **ZM, double **ZM1, double **ZBBL, double **ZBBR, double **p, int samples, int *seq_int){
  ...
  tbacktrackSP2ZBB(i+1,j-1,Z,ZB,ZBB,ZM,ZM1,ZBBL,ZBBR,p,aux,seq_int);
  ...
  tbacktrackSP2ZBBL(i+1,j-1,Z,ZB,ZBB,ZM,ZM1,ZBBL,ZBBR,p,aux,seq_int);
  ...
  tbacktrackSP2ZBBR(i+1,j-1,Z,ZB,ZBB,ZM,ZM1,ZBBL,ZBBR,p,aux,seq_int);
...
int tbacktrackSP2ZB(int i, int j, double **Z, double **ZB, double **ZBB, double **ZM, double **ZM1, double **ZBBL, double **ZBBR, double **p, int samples, int *seq_int){
  ...
  aux = floor(samples*((cbp[ij_index]*ZBB[i][j])/ZB[i][j]));
  tbacktrackSP2ZBB(i,j,Z,ZB,ZBB,ZM,ZM1,ZBBL,ZBBR,p,aux,seq_int);
  ...
  */
  kj_index =  d1[GetIndex(seq_int[i+2],seq_int[j-1])];
  aux = floor(samples*((cbp[kj_index]*ZBBL[i][j]*exp(-1*(GetLeftBulgeEnergy(i,j,i+2,seq_int)/RT)))/ZB[i][j]));
  tbacktrackSP2ZBBL(i,j,Z,ZB,ZBB,ZM,ZM1,ZBBL,ZBBR,p,aux,seq_int);

  for(k = i+3; (k <= j-SIGMA-2); k++){
    kj_index = d1[GetIndex(seq_int[k],seq_int[j-1])];
    aux = floor(samples*((cbp[kj_index]*ZB[k][j-1]*exp(-1*(GetLeftBulgeEnergy(i,j,k,seq_int)/RT)))/ZB[i][j]));
    tbacktrackSP2ZB(k,j-1,Z,ZB,ZBB,ZM,ZM1,ZBBL,ZBBR,p,aux,seq_int);
  }

  ik_index = d1[GetIndex(seq_int[i+1],seq_int[j-2])];
  aux = floor(samples*((cbp[ik_index]*ZBBR[i][j]*exp(-1*(GetRightBulgeEnergy(i,j,j-2,seq_int)/RT)))/ZB[i][j]));
  tbacktrackSP2ZBBR(i,j,Z,ZB,ZBB,ZM,ZM1,ZBBL,ZBBR,p,aux,seq_int);
  
  for(k = j-3; (k>=i+SIGMA+2); k--){
    ik_index = d1[GetIndex(seq_int[i+1],seq_int[k])];
    aux = floor(samples*((cbp[ik_index]*ZB[i+1][k]*exp(-1*(GetRightBulgeEnergy(i,j,k,seq_int)/RT)))/ZB[i][j]));
    tbacktrackSP2ZB(i+1,k,Z,ZB,ZBB,ZM,ZM1,ZBBL,ZBBR,p,aux,seq_int);
  }
  ...
  for(k = i+SIGMA+3; k <= j-SIGMA-2; k++){
    aux = floor(samples*((ZM[i+1][k-1]*ZM1[k][j-1]*exp(-1*((MultiloopA+MultiloopB)/RT)))/ZB[i][j]));
    tbacktrackSP2ZM(i+1,k-1,Z,ZB,ZBB,ZM,ZM1,ZBBL,ZBBR,p,aux,seq_int);
    tbacktrackSP2ZM1(k,j-1,Z,ZB,ZBB,ZM,ZM1,ZBBL,ZBBR,p,aux,seq_int);
  }
  //printf("Nothing? rd = %f\n",rd);
}
...
int tbacktrackSP2ZM1(int i, int j, double **Z, double **ZB, double **ZBB, double **ZM, double **ZM1,  double **ZBBL, double **ZBBR, double **p, int samples, int *seq_int){
  ...
  for(k = i+SIGMA+1; k <= j; k++){
    ...
    tbacktrackSP2ZB(i,k,Z,ZB,ZBB,ZM,ZM1,ZBBL,ZBBR,p,aux,seq_int);
  }
...
int tbacktrackSP2ZM(int i, int j, double **Z, double **ZB, double **ZBB, double **ZM, double **ZM1,  double **ZBBL, double **ZBBR, double **p, int samples, int *seq_int){
  ...
  for(k = i; k <= j-SIGMA-1; k++){
    ...
    tbacktrackSP2ZM1(k,j,Z,ZB,ZBB,ZM,ZM1,ZBBL,ZBBR,p,aux,seq_int);
  }

  for(k = i; k <= j-SIGMA-1; k++){
    aux = floor(samples*((ZM[i][k]*ZM1[k+1][j]*exp(-1*(MultiloopB/RT)))/ZM[i][j]));
    tbacktrackSP2ZM(i,k,Z,ZB,ZBB,ZM,ZM1,ZBBL,ZBBR,p,aux,seq_int);
    tbacktrackSP2ZM1(k+1,j,Z,ZB,ZBB,ZM,ZM1,ZBBL,ZBBR,p,aux,seq_int);
  }
}
...
int tbacktrackSP2Z(int i, int j, double **Z, double **ZB, double **ZBB, double **ZM, double **ZM1,  double **ZBBL, double **ZBBR, double **p, int samples, int *seq_int){
  ...
  tbacktrackSP2Z(i,j-1,Z,ZB,ZBB,ZM,ZM1,ZBBL,ZBBR,p,aux,seq_int);
   
  aux = floor(samples*(cbp[ij_index]*ZB[i][j]/Z[i][j]));
  tbacktrackSP2ZB(i,j,Z,ZB,ZBB,ZM,ZM1,ZBBL,ZBBR,p,aux,seq_int);

  for(k = i+1;k<=j-SIGMA-1;k++){ 
    kj_index = d1[GetIndex(seq_int[k],seq_int[j])];
    aux = floor(samples*(cbp[kj_index]*Z[i][k-1]*ZB[k][j]/Z[i][j]));
    tbacktrackSP2Z(i,k-1,Z,ZB,ZBB,ZM,ZM1,ZBBL,ZBBR,p,aux,seq_int);
    tbacktrackSP2ZB(k,j,Z,ZB,ZBB,ZM,ZM1,ZBBL,ZBBR,p,aux,seq_int);
  }

}


double tcalculateProbs2(int * seq_int, int n, int samples){ 
  double **Z;
  double **ZB;
  double **ZBB;
  double **ZBBL;
  ...
  for (d = SIGMA+1; d <= n; d++){
    for(i=1; (i+d<=n); i++){
      ...
      ZBBL[j][i] = tcalculateSP2ZBBLpfns(seq_int,i,j,ZBB,ZB,ZM,ZM1,ZBBL,ZBBR);

      ZBBR[j][i] = tcalculateSP2ZBBRpfns(seq_int,i,j,ZBB,ZB,ZM,ZM1,ZBBL,ZBBR);
   
      ZBB[j][i] = tcalculateSP2ZBBpfns(seq_int,i,j,ZBB,ZB,ZM,ZM1,ZBBL,ZBBR);

      ZB[j][i] = tcalculateSP2ZBpfns(seq_int,i,j,ZBB,ZB,ZM,ZM1,ZBBL,ZBBR);

      ZM1[j][i] = tcalculateSP2ZM1pfns(seq_int,i,j,ZB,ZM1);

      ZM[j][i] = tcalculateSP2ZMpfns(seq_int,i,j,ZM1,ZM);
   
      Z[j][i] = tcalculateSP2Zpfns(seq_int,i,j,ZB,Z);
    }
  }
  ...
  //Sampling
  tbacktrackSP2Z(1,n,Z,ZB,ZBB,ZM,ZM1,ZBBL,ZBBR,p,samples,seq_int);
